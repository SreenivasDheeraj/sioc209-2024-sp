...
def add_climate_features(dataset):
    # Assuming dataset is an xarray.Dataset containing 'pr' and 'diurnal_temperature_range'
    dataset['mean_pr'] = dataset['pr'].mean(dim=['lat', 'lon'])
    dataset['mean_dtr'] = dataset['diurnal_temperature_range'].mean(dim=['lat', 'lon'])
    return dataset

def get_mae(truth, pred):
    """Calculate the Mean Absolute Error accounting for spatial coordinates."""
    # Assuming `truth` and `pred` are xarray DataArray objects with dimensions [time, lat, lon]
    # This will weight the MAE by the cosine of the latitude for geographical relevance
    weights = np.cos(np.deg2rad(truth.lat))
    mae = abs(truth - pred).weighted(weights).mean(dim=['time', 'lat', 'lon']).data
    return float(mae)

def get_r2(truth, pred):
    """Calculate the R-squared value accounting for spatial coordinates."""
    # Flatten the arrays to fit sklearn's r2_score format
    truth_flattened = truth.values.flatten()
    pred_flattened = pred.values.flatten()
    r2 = r2_score(truth_flattened, pred_flattened)
    return r2
